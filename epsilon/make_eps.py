...
def gaussian(freq, pole, smear):
    return 1.0/(smear*math.sqrt(2.0*math.pi))*math.exp(-1.0/2.0*((freq-pole)**2/smear**2))
...
parser.add_argument('-st', help="smearing type (lorentzian | gaussian)",type=str , default="lorentzian", dest="sm_type")
...
if args.sm_type == "lorentzian":
    smearing=lorentzian
elif args.sm_type == "gaussian":
    smearing=gaussian
else:
    print(" Error unknown smearing !! ")
    exit(0)
...
for iw in range(args.ensteps):
    ...
    for ixc in range(data.shape[0]):
        eps[iw,1]=eps[iw,1]+smearing(w, data[ixc][0], args.smearing)*data[ixc][1]
        jdos[iw,1]=jdos[iw,1]+smearing(w, data[ixc][0], args.smearing)
        if args.mu != -1.0:
            exc_en=data[ixc][0]-data[0][0]+args.mu
            lum[iw,1]=lum[iw,1]+smearing(w, data[ixc][0], args.smearing)*data[ixc][1]*bose(exc_en,temp)
